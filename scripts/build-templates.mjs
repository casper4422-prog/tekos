// Build digit templates by AUTO-LABELING: the 2-column reader reads bases
// correctly, so each base cell's value is known -> label its leading digit
// glyphs and average into per-digit templates. Dev tool. -> /tmp/templates.json
import sharp from 'sharp';
const NW=16,NH=24;
const GT={ // fixture -> [row0L,row0R,row1L,row1R,row2L,row2R,row3L,row3R] base values (HP,WGT,STA,MEL,OXY,Speed,FOOD,Craft)
 'static/ocr-samples/giga.jpg':[51,47,53,50,55,0,47,0],
 'static/ocr-samples/mororex.png':[53,55,55,56,32,0,42,0],
 'static/ocr-samples/4wolf.webp':[65,59,51,56,61,0,52,0],
 'static/ocr-samples/lightning-wyvern.png':[59,62,52,57,53,0,50,0],
};
...
for(const FILE of Object.keys(GT)){
  ...
  const gt=GT[FILE];let labeled=0;
  for(let i=0;i<stat.length;i++){for(let s=0;s<2;s++){const cell=stat[i][s];if(!cell)continue;const val=String(gt[i*2+s]);const bcc=baseCCs(cell);if(bcc.length===val.length){for(let k=0;k<bcc.length;k++){buckets[+val[k]].push(normGlyph(bin,w,bcc[k]));labeled++;}}}}
  console.log(FILE.split('/').pop(),'labeled',labeled);
}
// average per digit -> template (cell=1 if >40% of samples have it)
const templates={};for(let d=0;d<10;d++){const arr=buckets[d];if(!arr.length){console.log('digit',d,'NO SAMPLES');continue;}const acc=new Float32Array(NW*NH);for(const g of arr)for(let i=0;i<NW*NH;i++)acc[i]+=g[i];const t=new Uint8Array(NW*NH);for(let i=0;i<NW*NH;i++)t[i]=acc[i]/arr.length>0.4?1:0;templates[d]=Array.from(t);console.log('digit',d,'samples',arr.length);}
const fs=await import('fs');fs.writeFileSync('/tmp/templates.json',JSON.stringify({NW,NH,templates}));
// montage templates for visual check
const SC=4,GAP=4,cw=NW*SC+GAP,MW=10*cw+GAP,MH=NH*SC+2*GAP;const img=Buffer.alloc(MW*MH,200);
for(let d=0;d<10;d++){const t=templates[d];if(!t)continue;const cx=d*cw+GAP;for(let y=0;y<NH;y++)for(let x=0;x<NW;x++){const v=t[y*NW+x]?0:255;for(let yy=0;yy<SC;yy++)for(let xx=0;xx<SC;xx++)img[(GAP+y*SC+yy)*MW+(cx+x*SC+xx)]=v;}}
await sharp(img,{raw:{width:MW,height:MH,channels:1}}).png().toFile('/tmp/templates.png');
console.log('-> /tmp/templates.json + /tmp/templates.png (0-9 left to right)');
