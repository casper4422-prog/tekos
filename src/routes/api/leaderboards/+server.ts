...
export const GET: RequestHandler = async ({ url }) => {
	...
	if (type === 'players') {
		const users = await db.user.findMany({
			select: { id:true, nickname:true, email:true, createdAt:true, _count: { select: { creatures:true, friendsInitiated:true } } },
			orderBy: { creatures: { _count: 'desc' } }, take: 50
		});
		return json(users.map((u, i) => ({ rank: i+1, id: u.id, name: u.nickname ?? u.email, specimens: u._count.creatures, friends: u._count.friendsInitiated })));
	}
