...
// Steam News API for ARK: Survival Ascended (App ID 2399830) — no auth required
...
export const GET: RequestHandler = async () => {
	...
	try {
		...
		const parsed = items.map((item: Record<string,unknown>) => ({
			...
			description: String(item.contents ?? '').replace(/<[^>]+>/g, '').slice(0, 220),
